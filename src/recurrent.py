...
class HanRnnSeq2Seq(object):
    def __init__(self, args, vocab):
        ...
        self.model_name = 'lstm_han_chatbot_epoch{:02d}_loss{:.3f}.h5' # args.model_name
        self.n_train_examples = args.n_train_examples
        self.n_valid_examples = args.n_valid_examples
        self.train_file = args.train_file
        self.valid_file = args.valid_file
    ...
    def build_model(self):
        word_level_input = Input(shape=(None,))
        context_level_input = Input(shape=(None, None))
        response_input = Input(shape=(None,))

        # Putting together word-level encoder
        embed_layer = Embedding(input_dim=self.vocab_size, output_dim=self.embedding_dim, mask_zero=True)
        bidi_encoder1 = Bidirectional(self.rec_cell(units=self.encoder_dim // 2, return_sequences=True))
        bidi_encoder2 = Bidirectional(self.rec_cell(units=self.encoder_dim // 2, return_sequences=True))
        att_layer = AttLayer(attention_dim=self.encoder_dim)

        word_embedded = embed_layer(word_level_input)
        h_word_out = bidi_encoder1(word_embedded)
        h_word_out = bidi_encoder2(h_word_out)
        h_att_out = att_layer(h_word_out)
        word_level_encoder = Model(inputs=word_level_input, outputs=h_att_out)

        # Context-level encoder
        top_encoder = self.rec_cell(units=self.encoder_dim, return_sequences=True)
        top_att_layer = AttLayer(attention_dim=self.encoder_dim)
        context_level_encoder = TimeDistributed(word_level_encoder)(context_level_input)
        # h_out = top_encoder(context_level_encoder)
        # h_context_att = top_att_layer(h_out)

        self.model = Model(inputs=context_level_input, outputs=context_level_encoder)
        self.model.summary()
